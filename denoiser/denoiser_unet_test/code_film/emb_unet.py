...
class FiLM(nn.Module):
    ...
    def forward(self, img, x):
        ...
        gamma = torch.unsqueeze(vec[:, :, 0], dim = -1)
        beta = torch.unsqueeze(vec[:, :, 1, :], dim = -1)
...
class EncBlock(nn.Module):
    def __init__(self, ch_in, ch_out, downsample = True):
        ...
        self.ds = downsample
        self.downsample = nn.Conv2d(ch_in, ch_in, kernel_size = 3, stride = 2, padding = 1)
        ...
        self.block2 = nn.Sequential(
            nn.Conv2d(ch_out, kernel_size = 3, stride = 1, padding = 1),
            nn.InstanceNorm2d(ch_out, affine = True),
            nn.ReLU(inplace = True)
        )
    def forward(self, x):
...
class DecBlock(nn.Module):
    def __init__(self, ch_in, ch_out, upsample = True):
        ...
        self.us = upsample
        self.upsample = nn.ConvTranspose2d(ch_in, ch_in, kernel_size = 3, stride = 2, padding = 1)
        self.block1 = nn.Sequential(
...
class EmbUNet(nn.Module):
    def __init__(self, ch_in = 1, scale = 32):
        ...
        self.down2 = EncBlock(scale * 2, scale * 4, downsample = True)
        self.se1 = SELayer(scale * 4, 16)
        self.down3 = EncBlock(scale * 4, scale * 8, downsample = True)
        self.se2 = SELayer(scale * 4, 16)

        self.bridge = EncBlock(scale * 8, scale * 8, downsample = True)

        self.up1 = DecBlock(scale * 8, scale * 4, upsample = True)
        self.up2 = DecBlock(scale * 4, scale * 2, upsample = True)
        self.up3 = DecBlock(scale * 2, scale, upsample = True)

        self.tail = nn.Conv2d(scale, ch_in, kernel_size = 1, stride = 1, padding = 0)
    def forward(self, x):
